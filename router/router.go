...
func (r *Router) initRoutes() {
	...
	sma := `{"message":"👊 welcome to $PASSWORD_MANAGER_NAME api","api":"$PASSWORD_MANAGER_NAME Core API","version": "v0.0.0"}`
	...
	authRouter.HandleFunc("/fmp", api.ForgotMasterPassword(r.store)).Methods(http.MethodPost)
	authRouter.HandleFunc("/fmp-send", api.SendFMP(r.store)).Methods(http.MethodPost)
