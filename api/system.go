...
func Export(s db.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		csvModels = append(csvModels, csvModel{"Logins", logins},
		...
		w.Header().Set("Content-Disposition", "attachment;filename=$PASSWORD_MANAGER_NAME.zip")
		w.Write(zipFile)
	}
}
...
func upload(r *http.Request) (*os.File, error) {
	...
	tempFile, err := ioutil.TempFile("/tmp", "$PASSWORD_MANAGER_NAME-import-*.csv")
